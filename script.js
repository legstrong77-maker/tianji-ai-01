...
function postToGoogleBackend(score, element, baziSummary) {
    ...
    // We want to upload the image WITH the AI blue lines on it.
    // So we merge the original image with the face-canvas.
    const imgEl = document.getElementById('scanned-image');
    const overlayCanvas = document.getElementById('face-canvas');
    
    const mergeCanvas = document.createElement('canvas');
    mergeCanvas.width = imgEl.naturalWidth;
    mergeCanvas.height = imgEl.naturalHeight;
    const ctx = mergeCanvas.getContext('2d');
    
    // Draw original picture
    ctx.drawImage(imgEl, 0, 0, mergeCanvas.width, mergeCanvas.height);
    
    // Draw the AI landmarks on top (scaled up to match natural resolution)
    if(overlayCanvas) {
        ctx.drawImage(overlayCanvas, 0, 0, mergeCanvas.width, mergeCanvas.height);
    }
    
    // Compress and get base64 
    const compositeBase64 = mergeCanvas.toDataURL('image/jpeg', 0.85);
    ...
    fetch(backendUrl, {
        method: 'POST',
        mode: 'no-cors', // Prevents console error when Google Apps Script redirects
        headers: {
